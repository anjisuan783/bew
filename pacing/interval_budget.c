...
void set_target_rate_kbps(interval_budget_t* budget, int target_rate_kbps)
{
	...
	budget->max_bytes_in_budget = (k_window_ms * target_rate_kbps >> 3);
	// -budget->max_bytes_in_budget <= budget->bytes_remaining <= budget->max_bytes_in_budget
...
{
	int bytes = (budget->target_rate_kbps * delta_ts >> 3);
	if (budget->bytes_remaining < 0 || budget->can_build_up_underuse == 0)
