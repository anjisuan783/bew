...
#else 
	return Clock::GetRealTimeClock()->TimeInMilliseconds();
#endif
}
...
int su_tcp_listen_create(uint16_t port, su_socket* fd)
{
	...
	buf_size = 16 * 1024;
	setsockopt(s, SOL_SOCKET, SO_RCVBUF, (const char*)&buf_size, sizeof(int32_t));
	setsockopt(s, SOL_SOCKET, SO_SNDBUF, (const char*)&buf_size, sizeof(int32_t));
