...
HRESULT DS_Utils::EnumAllCaptureDevice(std::vector<SCaptureDevice>& capDev) {
  ...
  if (FAILED(hr = EnumCaptureDevice(capDevVideo, true)))
    return hr;

  if (FAILED(hr = hr = EnumCaptureDevice(capDevAudio, false)))
    return hr;
...
HRESULT DS_Utils::EnumCaptureDevice(std::vector<SCaptureDevice>& capDev,
                                    bool bVideo) {
  HRESULT hr = E_FAIL;
  ...
  hr = pDevEnum->CreateClassEnumerator(
      bVideo ? CLSID_VideoInputDeviceCategory : CLSID_AudioInputDeviceCategory,
      &pIEnumMoniker, 0);
  ...
  while (pIEnumMoniker->Next(1, &pMoniker, NULL) == S_OK) {
    ...
    if (SUCCEEDED(hr = pPropBag->Read(L"Description", &varNameDescription, 0)))
      cd.Description = varNameDescription.bstrVal;

    if (SUCCEEDED(hr = pPropBag->Read(L"FriendlyName", &varNameFriendly, 0)))
      cd.FriendlyName = varNameFriendly.bstrVal;

    if (SUCCEEDED(hr = pPropBag->Read(L"DevicePath", &varNamePath, 0)))
      cd.DevicePath = varNamePath.bstrVal;

    try {
      CComPtr<IBaseFilter> pCap;
      if (SUCCEEDED(hr = pMoniker->BindToObject(0, 0, IID_IBaseFilter, (void**)&pCap))) {
        GUID guid = GUID_NULL;
              ...
              if (pMediaType) {
                if (pMediaType->majortype == MEDIATYPE_Video) {
                  ...
                  if (pHeader->AvgTimePerFrame > 0)
                    maxFPS = static_cast<int> (10000000/pHeader->AvgTimePerFrame);

                  RawVideoType rawType = kVideoUnknown;
                  //if (pMediaType->subtype == MEDIASUBTYPE_I420) {
                  //  rawType = kVideoI420;
                  /*}else*/ if (pMediaType->subtype == MEDIASUBTYPE_IYUV){
                    rawType = kVideoIYUV;
                    ...
                    rawType = kVideoMJPEG;
                  }else if (pMediaType->subtype == MEDIASUBTYPE_dvsl
                          || pMediaType->subtype == MEDIASUBTYPE_dvsd
                          || pMediaType->subtype == MEDIASUBTYPE_dvhd) { // If this is an external DV camera
                    rawType = kVideoYUY2;// MS DV filter seems to create this type
                  } else if (pMediaType->subtype == MEDIASUBTYPE_UYVY) { // Seen used by Declink capture cards
                    rawType = kVideoUYVY;
                  } /*else if (pMediaType->subtype == MEDIASUBTYPE_HDYC) { // Seen used by Declink capture cards. Uses BT. 709 color. Not entiry correct to use UYVY. http://en.wikipedia.org/wiki/YCbCr
                    rawType = kVideoUYVY;
                  } */else {
                  ...
                  SDeviceCapability capability{ 
                      pHeader->bmiHeader.biWidth, 
                      pHeader->bmiHeader.biHeight , 
                      maxFPS, rawType};

                  if (cd.capabilities.end() == std::find_if(cd.capabilities.begin(), cd.capabilities.end(), [&capability](const SDeviceCapability& p) {
                      return !(p != capability);
                  })) {
                    cd.capabilities.push_back(capability);
                  }
                }
...
HRESULT DS_Utils::EnumGraphFilters(
...
HRESULT DS_Utils::EnumPins(CComPtr<IBaseFilter> pFilter,
...
HRESULT DS_Utils::FindFilter(CComPtr<IGraphBuilder> pGraph,
                             CLSID classid,
                             CComPtr<IBaseFilter>& pFilter) {
  HRESULT hr = E_FAIL;
...
bool DS_Utils::IsPinConnected(CComPtr<IPin> pPin,
...
HRESULT DS_Utils::SetCaptureFilter_FriendlyName(CComPtr<IBaseFilter> pFilter,
                                                std::wstring name) {
  HRESULT hr = E_FAIL;
  BOOL bFound = FALSE;
...
HRESULT DS_Utils::ShowCaptureFilterPropPage(CComPtr<IBaseFilter> pFilter,
                                            HWND hWndParent,
                                            UINT x,
                                            UINT y) {
  HRESULT hr = E_FAIL;
  ...
  if (hr == S_OK) {
    ...
    hr =
        OleCreatePropertyFrame(hWndParent, x, y, NULL, 1, (IUnknown**)&pFilter,
...
HRESULT Graph::SeekToFirst() {
  ...
  LONGLONG pos = 0;
  if (m_pSeeking != NULL)
    hr = m_pSeeking->SetPositions(&pos, AM_SEEKING_AbsolutePositioning, NULL,
