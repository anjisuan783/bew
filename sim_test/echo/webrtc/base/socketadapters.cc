...
AsyncUDPAssociateSocket::AsyncUDPAssociateSocket(AsyncSocket* socket,
                                                 const SocketAddress& proxy,
                                                 const std::string& username,
                                                 const CryptString& password)
    : BufferedReadAdapter(socket, 1024), state_(SS_ERROR), proxy_(proxy),
...
AsyncUDPAssociateSocket::~AsyncUDPAssociateSocket() = default;

int AsyncUDPAssociateSocket::Associate(const SocketAddress& addr) {
  int ret;
  associate_ = addr;
  state_ = SS_INIT;
  BufferInput(true);
  ret = BufferedReadAdapter::Connect(proxy_);
  LOG_T(LS_INFO) << "AsyncUDPAssociateSocket::Associate(), Associate udp addr: " << addr.ToString() << ", ret = " << ret;
  // TODO: Set state_ appropriately if Connect fails.
  return ret;
}

int AsyncUDPAssociateSocket::Close() {
  state_ = SS_ERROR;
  associate_.Clear();
  relay_.Clear();
  return BufferedReadAdapter::Close();
...
Socket::ConnState AsyncUDPAssociateSocket::GetState() const {
  if (state_ < SS_TUNNEL) {
    return CS_CONNECTING;
  } else if (state_ == SS_TUNNEL) {
    return CS_CONNECTED;
...
void AsyncUDPAssociateSocket::OnConnectEvent(AsyncSocket* socket) {
  SendHello();
}

void AsyncUDPAssociateSocket::OnCloseEvent(AsyncSocket * socket, int err) {
  state_ = SS_ERROR;
  associate_.Clear();
  relay_.Clear();
...
void AsyncUDPAssociateSocket::ProcessInput(char* data, size_t* len) {
  RTC_DCHECK(state_ < SS_TUNNEL);
  ...
  if (state_ == SS_HELLO) {
    ...
    SendUDPAssociate();
  } else if (state_ == SS_UDP_ASSOCIATE) {
    uint8_t ver, rep, rsv, atyp;
    if (!response.ReadUInt8(&ver) ||
        !response.ReadUInt8(&rep) ||
        !response.ReadUInt8(&rsv) ||
    ...
    if (atyp == 1) {
      ...
      if (!response.ReadUInt32(&addr) ||
          !response.ReadUInt16(&port))
        return;
      relay_ = SocketAddress(addr, port);
    } else if (atyp == 3) {
      uint8_t len;
      std::string addr;
      if (!response.ReadUInt8(&len) ||
    ...
    state_ = SS_TUNNEL;
    LOG_T(LS_INFO) << "AsyncUDPAssociateSocket::ProcessInput(), Bound udp relay addr: " << relay_.ToString();
  }
  ...
  *len = response.Length();
  ...
  bool remainder = (*len > 0);
  BufferInput(false);
  SignalAssociateEvent(this, relay_);
...
void AsyncUDPAssociateSocket::SendHello() {
  ByteBufferWriter request;
  request.WriteUInt8(5);    // Socks Version
  if (user_.empty()) {
    request.WriteUInt8(1);  // Authentication Mechanisms
...
void AsyncUDPAssociateSocket::SendAuth() {
  ByteBufferWriter request;
  request.WriteUInt8(1);           // Negotiation Version
  request.WriteUInt8(static_cast<uint8_t>(user_.size()));
  request.WriteString(user_);      // Username
...
void AsyncUDPAssociateSocket::SendUDPAssociate() {
  ByteBufferWriter request;
  request.WriteUInt8(5);              // Socks Version
  request.WriteUInt8(3);              // UDP Associate
  request.WriteUInt8(0);              // Reserved
  request.WriteUInt8(1);              // IPV4
  request.WriteUInt32(0);             // Destination IP (Unspecified IP Address)
  request.WriteUInt16(0);             // Destination Port (Unspecified Port)
  DirectSend(request.Data(), request.Length());
  state_ = SS_UDP_ASSOCIATE;
}

void AsyncUDPAssociateSocket::Error(int error) {
  state_ = SS_ERROR;
  BufferInput(false);
  Close();
  SetError(SOCKET_EACCES);
...
AsyncSocksProxyUDPSocket::AsyncSocksProxyUDPSocket(AsyncSocket* udp_socket,
                                                   AsyncSocket* tcp_socket,
                                                   const SocketAddress& proxy,
                                                   const std::string& username,
                                                   const CryptString& password)
  : AsyncSocketAdapter(udp_socket),
    associate_socket_(tcp_socket, proxy, username, password) {
  associate_socket_.SignalAssociateEvent.connect(this, &AsyncSocksProxyUDPSocket::OnAssociateConnectEvent);
  associate_socket_.SignalCloseEvent.connect(this, &AsyncSocksProxyUDPSocket::OnAssociateCloseEvent);
  associate_socket_.Associate(GetLocalAddress());
}

AsyncSocksProxyUDPSocket::~AsyncSocksProxyUDPSocket() {
  associate_socket_.Close();
}

int AsyncSocksProxyUDPSocket::SendTo(const void* pv,
                                     size_t cb,
                                     const SocketAddress& addr) {
  if (associate_socket_.GetState() != CS_CONNECTED || relay_.IsNil()) {
    socket_->SetError(EWOULDBLOCK);
    return -1;
  }

  // add relay header for udp data
  send_buffer_.Clear();
  send_buffer_.WriteUInt16(0);              // RSV Reserved X'0000'
  send_buffer_.WriteUInt8(0);               // FRAG
  if (addr.IsUnresolvedIP()) {
    std::string hostname = addr.hostname();
    send_buffer_.WriteUInt8(3);             // DOMAINNAME
    send_buffer_.WriteUInt8(static_cast<uint8_t>(hostname.size()));
    send_buffer_.WriteString(hostname);     // Destination Hostname
  } else {
    send_buffer_.WriteUInt8(1);             // IPV4
    send_buffer_.WriteUInt32(addr.ip());    // Destination IP
  }
  send_buffer_.WriteUInt16(addr.port());    // Destination Port

  int relay_header_len = send_buffer_.Length();
  send_buffer_.WriteBytes((const char *)pv, cb);
  int send_len = AsyncSocketAdapter::SendTo(send_buffer_.Data(), send_buffer_.Length(), relay_);
  send_len -= relay_header_len;
  return send_len;
}

int AsyncSocksProxyUDPSocket::RecvFrom(void* pv,
                                       size_t cb,
                                       SocketAddress* paddr,
                                       int64_t* timestamp) {
  if (associate_socket_.GetState() != CS_CONNECTED) {
    socket_->SetError(EWOULDBLOCK);
    return -1;
  }

  // remove relay header for udp data
  int recv_len = AsyncSocketAdapter::RecvFrom(pv, cb, paddr, timestamp);

  ByteBufferReader recv_data((char*)pv, recv_len);
  uint16_t rsv;
  uint8_t frag, atyp;
  if (!recv_data.ReadUInt16(&rsv) ||
      !recv_data.ReadUInt8(&frag) ||
      !recv_data.ReadUInt8(&atyp))
    return 0;
  ...
  if (atyp == 1) {
    uint32_t addr;
    if (!recv_data.ReadUInt32(&addr) ||
        !recv_data.ReadUInt16(&port))
      return 0;
    recv_addr = SocketAddress(addr, port);
  } else if (atyp == 3) {
    uint8_t len;
    std::string addr;
    if (!recv_data.ReadUInt8(&len) ||
        !recv_data.ReadString(&addr, len) ||
        !recv_data.ReadUInt16(&port))
      return 0;
    recv_addr = SocketAddress(addr, port);
  } else if (atyp == 4) {
    std::string addr;
    if (!recv_data.ReadString(&addr, 16) ||
        !recv_data.ReadUInt16(&port))
      return 0;
    recv_addr = SocketAddress(addr, port);
  } else {
  ...
  *paddr = recv_addr;
  recv_len = recv_data.Length();
  memmove(pv, recv_data.Data(), recv_len);
  return recv_len;
}

Socket::ConnState AsyncSocksProxyUDPSocket::GetState() const {
  return associate_socket_.GetState();
}

void AsyncSocksProxyUDPSocket::OnAssociateConnectEvent(AsyncUDPAssociateSocket* socket,
                                                       const SocketAddress& addr) {
  RTC_DCHECK(socket == &associate_socket_);

  LOG_T(LS_INFO) << "AsyncSocksProxyUDPSocket::OnAssociateConnectEvent(), Associate success, get udp relay addr: " << addr.ToString()
                 << " with local addr: " << GetLocalAddress().ToString();
  relay_ = addr;
}

void AsyncSocksProxyUDPSocket::OnAssociateCloseEvent(AsyncSocket* socket, int error) {
  RTC_DCHECK(socket == &associate_socket_);

  LOG_T(LS_INFO) << "AsyncSocksProxyUDPSocket::OnAssociateCloseEvent(), Associate close, clear udp relay addr: " << relay_.ToString()
                 << " with local addr: " << GetLocalAddress().ToString() << ", error: " << error;
  relay_.Clear();
}
