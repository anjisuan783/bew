...
// Implements a UDP socket adapter that speaks the SOCKS proxy protocol.
class AsyncUDPAssociateSocket : public BufferedReadAdapter {
 public:
  AsyncUDPAssociateSocket(AsyncSocket* socket, const SocketAddress& proxy,
    const std::string& username, const CryptString& password);
  ~AsyncUDPAssociateSocket() override;

  int Close() override;
  ConnState GetState() const override;

  int Associate(const SocketAddress& addr);
  sigslot::signal2<AsyncUDPAssociateSocket*,
                   const SocketAddress&>  SignalAssociateEvent;
 ...
 protected:
  ...
  void SendHello();
  void SendAuth();
  void SendUDPAssociate();
  void Error(int error);

 private:
  enum State {
    SS_INIT, SS_HELLO, SS_AUTH, SS_UDP_ASSOCIATE, SS_TUNNEL, SS_ERROR
  };
  State state_;
  SocketAddress proxy_, associate_, relay_;
  std::string user_;
  CryptString pass_;
  RTC_DISALLOW_COPY_AND_ASSIGN(AsyncUDPAssociateSocket);
};

class AsyncSocksProxyUDPSocket : public AsyncSocketAdapter {
 public:
  AsyncSocksProxyUDPSocket(AsyncSocket* udp_socket, AsyncSocket* tcp_socket,
    const SocketAddress& proxy, const std::string& username, const CryptString& password);
 ...
 protected:
  void OnAssociateConnectEvent(AsyncUDPAssociateSocket* socket, const SocketAddress& addr);
  void OnAssociateCloseEvent(AsyncSocket* socket, int error);

 private:
  AsyncUDPAssociateSocket associate_socket_;
  SocketAddress relay_;
  ByteBufferWriter send_buffer_;
  RTC_DISALLOW_COPY_AND_ASSIGN(AsyncSocksProxyUDPSocket);
};
