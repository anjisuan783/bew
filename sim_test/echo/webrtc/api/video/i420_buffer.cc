...
I420Buffer::I420Buffer(int width, int height)
    : I420Buffer(width, height, width, (width + 1) >> 1, (width + 1) >> 1) {
}
...
I420Buffer::I420Buffer(int width,
  ...
  RTC_DCHECK_GE(stride_u, (width + 1) >> 1);
  RTC_DCHECK_GE(stride_v, (width + 1) >> 1);
}
...
void I420Buffer::CropAndScaleFrom(
  ...
  const int crop_width =
      std::min<int>(src.width(), width() * src.height() / height());
  const int crop_height =
      std::min<int>(src.height(), height() * src.width() / width());
